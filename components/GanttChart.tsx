...
const getOffset = (date: Date, timelineStartDate: Date, context: GanttContextProps) => {
  ...
  if (context.range === 'daily') return parsedColumnWidth * fullColumns;
...
const DailyHeader: FC = () => {
  ...
  return gantt.timelineData.map((year) =>
    year.quarters.flatMap((quarter) => quarter.months).map((month, index) => (
...
export const GanttSidebarItem: FC<GanttSidebarItemProps> = ({ feature, onSelectItem, className }) => {
  ...
  const handleKeyDown: KeyboardEventHandler<HTMLDivElement> = (event) => {
    if (event.key === 'Enter') onSelectItem?.(feature.id);
  };
...
export const GanttSidebarHeader: FC = () => (
  <div
    ...
    style={{ height: 'var(--gantt-header-height)' }}
  >
    <p className="flex-1 truncate text-left">Phase</p>
...
export type GanttFeatureItemCardProps = Pick<GanttFeature, 'id'> & { children?: ReactNode };
  ...
  return (
    <div className={cn('relative flex w-max min-w-full py-0.5', className)} style={{ height: 'var(--gantt-row-height)' }}>
      <div
        ...
        style={{ height: 'calc(var(--gantt-row-height) - 4px)', width: Math.round(width), left: Math.round(offset) }}
      >
        {onMove && (
        ...
        {onMove && (
          <DndContext sensors={[mouseSensor]} modifiers={[restrictToHorizontalAxis]} onDragMove={handleRightDragMove} onDragEnd={onDragEnd}>
            <GanttFeatureDragHelper direction="right" featureId={feature.id} date={endAt ?? addRange(startAt, 2)} />
          </DndContext>
        )}
      </div>
    </div>
...
export const GanttMarker: FC<GanttMarkerProps & { onRemove?: (id: string) => void; className?: string }> = ({
  ...
  return (
    <div
      ...
      style={{ width: 0, transform: `translateX(calc(var(--gantt-column-width) * ${offset} + ${innerOffset}px))` }}
    >
      <ContextMenu>
...
export const GanttProvider: FC<GanttProviderProps> = ({ zoom = 100, range = 'monthly', onAddItem, children, className }) => {
  ...
  useEffect(() => {
    ...
    return () => { if (scrollRef.current) scrollRef.current.removeEventListener('scroll', handleScroll); };
  }, [handleScroll]);
...
export const GanttToday: FC<GanttTodayProps> = ({ className }) => {
  ...
  return (
    <div
      ...
      style={{ width: 0, transform: `translateX(calc(var(--gantt-column-width) * ${offset} + ${innerOffset}px))` }}
    >
      {/* ML-branded today marker */}
      <div
        className={cn(
          'group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md px-2 py-1 text-xs font-semibold',
          className
        )}
        style={{ background: '#000F1E', color: '#FFFFFF' }}
      >
        {label}
        <span className="max-h-[0] overflow-hidden opacity-80 transition-all group-hover:max-h-[2rem]">
          {formatDate(date, 'MMM dd, yyyy')}
      ...
      <div className="h-full w-px" style={{ background: '#000F1E', opacity: 0.4 }} />
    </div>
  );
};
